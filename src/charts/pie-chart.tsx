...
export const DEFAULT_HOVER_OFFSET = 16;
...
export interface PieChartProps {
  ...
  /** Programmatically highlight a slice (persists when not hovering) */
...
const PieChartCore = memo(function PieChartCore({
  ...
  // When transitioning from controlled → uncontrolled, clear any stale hover state
  // so a previously hovered segment doesn't re-highlight unexpectedly.
  ...
  // Mouse hover takes precedence; fall back to selectedIndex when not hovering
  const effectiveHoveredIndex = hoveredIndex ?? selectedIndex ?? null;
    ...
    <PieProvider value={contextValue}>
      ...
      >
        ...
        <svg
          aria-hidden="true"
          height={size}
          style={{ gridArea: "1 / 1", contain: "layout style", overflow: "visible" }}
          width={size}
        >
