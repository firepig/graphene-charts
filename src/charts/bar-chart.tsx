...
export interface BarChartProps {
  ...
  stackGap?: number;
  /** Stacking mode: "normal" stacks raw values; "percent" normalizes to 0-100. Default: "normal" */
...
const ChartCore = memo(function ChartCore({
  ...
  // Compute domain min/max considering stacking and stackMode
  const { domainMin, domainMax } = useMemo(() => {
    if (stackMode === "percent") {
      // Percent mode always spans 0–100
      return { domainMin: 0, domainMax: 100 };
    }

    if (stacked) {
      // For stacked bars, track separate positive and negative cumulative sums
      let max = 0;
      let min = 0;
      for (const d of data) {
        let posSum = 0;
      ...
      return { domainMin: min, domainMax: max || 100 };
    }

    // For grouped bars, find min and max single values
    let max = 0;
    let min = 0;
    for (const line of lines) {
      for (const d of data) {
    ...
    return { domainMin: min, domainMax: max || 100 };
  }, [data, lines, stacked, stackMode]);

  // Value scale (linear) - for the value axis
  const valueScale = useMemo(() => {
    const range = isHorizontal ? [0, innerWidth] : [innerHeight, 0];
    const paddingFactor = 1.1;
    const scaledMin = domainMin < 0 ? domainMin * paddingFactor : domainMin;
    const scaledMax = domainMax > 0 ? domainMax * paddingFactor : domainMax;
    return scaleLinear({
      range,
      domain: [scaledMin, scaledMax],
      nice: true,
    });
  }, [innerWidth, innerHeight, domainMin, domainMax, isHorizontal]);
  ...
  const yScales = useMemo(() => {
    ...
    return buildYScalesForLines({
      ...
      resolveDomain: (dataKeys) => {
        ...
        const paddingFactor = 1.1;
        const scaledMin = min < 0 ? min * paddingFactor : min;
        const scaledMax = (max || 100) * paddingFactor;
  ...
  // Compute stack offsets for stacked bars (supports negative values and percent mode)
  const stackOffsets = useMemo(() => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // For percent mode, compute the total (absolute) so we can normalize
      let total = 0;
      if (stackMode === "percent") {
        for (const line of lines) {
          const value = d[line.dataKey];
      ...
      // Track separate positive and negative running totals for mixed-sign stacks
      let posCumulative = 0;
      let negCumulative = 0;
      for (const line of lines) {
        const rawValue = d[line.dataKey];
        ...
        const value =
          stackMode === "percent"
            ...
            : rawValue;
        if (value >= 0) {
          pointOffsets.set(line.dataKey, posCumulative);
          posCumulative += value;
        } else {
          pointOffsets.set(line.dataKey, negCumulative);
          negCumulative += value;
        }
      }
